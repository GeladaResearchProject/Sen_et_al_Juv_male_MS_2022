...
old_values                   <- read.csv("/Users/sharmisen/Desktop/CURRENT MANUSCRIPT PROJECTS/1.T VALIDATION PAPER/T_MS_Input_Files/epiandro_mpbio_comparison.csv")
...
df_models <- read.csv("/Users/sharmisen/Desktop/CURRENT MANUSCRIPT PROJECTS/1.T VALIDATION PAPER/T_MS_Input_files/androgen_dataset_6.25.2022.csv")
...
#plotting T levels with disperser category 

#residual T levels around dispersal overall 
subset_disp$ng.g <- log(subset_disp$ng.g)
revised_disp_data <- subset_disp %>% group_by(ID, dis_cat,time_dis) %>% 
                     summarise(mean_ng.g = mean(ng.g, na.rm = T), sd_ng.g = sd(ng.g, na.rm = T), 
                     num_samples = n()) %>% 
                     group_by(dis_cat, time_dis)

head(revised_disp_data)

revised_disp_data2 <- revised_disp_data %>% group_by(dis_cat, time_dis) %>% 
                     summarise(mean_cat_ng.g = mean(mean_ng.g, na.rm = T), se_cat_ng.g = sd(mean_ng.g, na.rm = T)/sqrt(n()), 
                     num_males = n()) %>% 
                     group_by(dis_cat, time_dis)

#plotting bars with +/- standard errors
jpeg("HB_RevFig4.jpg" , height = 15, width = 18, units = "cm", res = 340)
ggplot(data = revised_disp_data2, aes(x = factor(time_dis), y = mean_cat_ng.g, fill = dis_cat)) + 
  geom_bar(stat = "identity") + theme_pubr(base_size = 18) + ylim(c(0,8)) +
  xlab("Months relative to dispersal") + ylab("log fAMs (ng/g)") + 
  theme(legend.background = element_rect(color = NA)) + geom_vline(xintercept = 4, size = 0.75, linetype = "dashed", col = "black") +
  geom_errorbar(aes(ymin= mean_cat_ng.g - se_cat_ng.g, ymax= mean_cat_ng.g + se_cat_ng.g), width=.2,
                 position=position_dodge(.9)) + facet_wrap(.~ dis_cat) + 
  scale_fill_manual(" ", values = c("#E69F00",makeTransparent("#332288",220),"#999999")) + 
  geom_text(aes(label = num_males, y = 0.75), vjust = 2, size = 4.5, fontface = "italic" , color = "white")
dev.off()
...
paired_data <- read.csv("/Users/sharmisen/Desktop/CURRENT MANUSCRIPT PROJECTS/1.T VALIDATION PAPER/T_MS_Input_Files/paired_dispersal_data.csv")
paired_data  
...
ggpaired(paired_data, cond1 = "before", cond2 =  "after", xlab = "Dispersal", 
         ylab = "log fAMs(ng/g)", line.color = makeTransparent("black",200), 
